    ...
    <>
      ...
      <section className="container-luxe pb-32">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border">
          {features.map((f, i) => (
            <div key={i} className="group bg-onyx-900 p-8 hover:bg-onyx-800 transition-colors duration-500 relative">
              <div className="absolute top-6 right-6 text-[10px] text-muted-foreground tracking-widest">
                {String(i + 1).padStart(2, "0")}
              </div>
              <div className="w-11 h-11 mb-6 border border-primary/40 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500">
                <f.icon size={16} />
              </div>
